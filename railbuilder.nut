class Path {
	...
	function GetMeetsTiles() {
		...
		if (straightFork != prevprev) {
			return [
				[cur, prev, straightFork, prevprev]
			];
		} else {
			local result = [];
	...
	function BuildDepotForRail() {
		...
		while (path != null) {
			...
			if (i == 5) {
				if (AIMap.DistanceManhattan(p[2], p[1]) > 1 || AIMap.DistanceManhattan(p[2], p[3]) > 1) {} else {
	...
	function GetSlopes(length, endTile = null) {
		...
		for (; endPath != null && (endPrev == null || endPrev != endTile); endPrevprev = endPrev, endPrev = endPath.GetTile(), endPath = endPath.GetParent()) {

			for (; startPath != null && endPoint + length > startPoint; startPrevprev = startPrev, startPrev = startPath.GetTile(), startPath = startPath.GetParent()) {
...
class RailBuilder extends Construction {
		...
		while (path != null) {
			...
			if (prev != null && AIMap.DistanceManhattan(prev, path.GetTile()) > 1) {
				...
				if (!AIRail.AreTilesConnected(prevprev, prev, path.GetTile()) || !AICompany.IsMine(AITile.GetOwner(prev))) {
					...
					AddRollback({
						name = "rail",
						tiles = [prevprev, prev, path.GetTile()]
					});
	...
	function TryDemolishUnderBridge(p1, p2) {
		...
		while (t0 != t1) {
			if (AIRoad.IsRoadTile(t0) || AIRail.IsRailTile(t0) || AITile.IsWaterTile(t0) || AIBridge.IsBridgeTile(t0) || AITunnel.IsTunnelTile(t0)) {} else if (!AITile.IsBuildable(t0)) {
	...
	function RebuildForHomeward(buildedPath) {
		...
		for (; path != null; prev = path.GetTile(), path = path.GetParent()) {
			...
			if (!HgTile.IsAroundCoast(revPrev) && !isAroundCoastRevNext) {
				HgTile.ForceLevelBound(revPrev, revNext, level, {
					lowerOnly = true
				});
			}
	...
	function ChangeBridge(prevprev, prev, next) {
		...
		local removed = [];
		if (tracks == AIRail.RAILTRACK_NE_SW) {} else if (tracks == AIRail.RAILTRACK_NW_SE) {} else {
...
class RailPathBuilder extends Construction {
	...
	static function GetStartArray(path) {
		...
		for (; path != null; prev3 = prev2, prev2 = prev, prev = path.GetTile(), path = path.GetParent()) {
			...
			if (d1 == 1 && d2 == 1 && !AIRail.AreTilesConnected(next, cur, prev)) {
				/*double depots地点からは分岐不可*/
				continue;
			}
			if (d2 == 1 && d3 == 1 && !AIRail.AreTilesConnected(cur, prev, prev2)) {
				/*double depots地点からは分岐不可*/
				continue;
			}
...
class ConstructionRailBuilder extends Construction {
	...
	function BuildDoubleDepots(path, interval, isFirstLine = false) {
		...
		foreach(built in path.BuildDoubleDepotInterval(interval, isFirstLine)) {
			...
			depotInfos.rawset(built.mainTiles[1], {
				depots = built.depots,
				mainTiles = built.mainTiles
			});
		}
		/*
		while(path != null) {
...
class TwoWayPtoPRailBuilder extends ConstructionRailBuilder {
	...
	function DoBuild() {
		...
		local goalsTiles = [
			[arrivalsTiles[l1 - 1], arrivalsTiles[l1 - 2], arrivalsTiles[l1 - 3], arrivalsTiles[l1 - 4]]
		];
		local startTiles = [
			[departuresTiles[0], departuresTiles[1], departuresTiles[2], departuresTiles[3]]
		];
		...
		local revStartTiles = [
			[revArrivalsTiles[l2 - 1], revArrivalsTiles[l2 - 2], revArrivalsTiles[l2 - 3], revArrivalsTiles[l2 - 4]]
		];
		local revGoalsTiles = [
			[revDeparturesTiles[0], revDeparturesTiles[1], revDeparturesTiles[2], revDeparturesTiles[3]]
		];
		// src => dest
