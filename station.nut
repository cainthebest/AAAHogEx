...
class RoadStation extends HgStation {
	...
	function GetBuildableScore() {
		...
		tileList.Valuate(function(tile) {
			return RoadRoute.used.map.rawin(tile) ? 1 : 0
		});
		tileList.KeepValue(1);
...
class TransferStation extends RailStation {
	...
	*/
	...
	function GetArrivalsTiles() {
		return [
			[At(1, platformLength + 1), At(1, platformLength)]
		];
	...
	function GetDepots() {
		return [
			[At(0, platformLength + 1), At(0, platformLength)]
		]; //[[At(-1,platformLength), At(0,platformLength)],[At(2,platformLength), At(1,platformLength)]];
	}
	...
	function Build(levelTiles = true, isTestMode = true) {
		...
		local l = GetTileListForLevelTiles([
			[1, -3],
			[1, platformLength + 2]
		]);
...
class SmartStation extends RailStation {
	...
	*/
	...
	function GetArrivalsTiles() {
		return [
			[At(0, platformLength + 5), At(0, platformLength + 4), At(0, platformLength + 3)]
		];
	}

	function GetDeparturesTiles() {
		return [
			[At(1, platformLength + 6), At(1, platformLength + 5), At(1, platformLength + 2)]
		];
	}
...
class TerminalStation extends DestRailStation {
	...
	function GetDeparturesTiles() {
		return [
			[At(platformNum + 1, platformLength + 2), At(platformNum, platformLength + 2)]
		];
	}
...
class SrcRailStation extends RailStation {
	...
	function Demolish() {
		...
		foreach(rail in GetRails()) {
			...
			if (!AIRail.RemoveRail(t1, t2, t3)) {
				HgLog.Warning("RemoveRail failed. " + HgTile(t1) + " "
					HgTile(t2) + " " + HgTile(t3) +
	...
	function GetArrivalsTiles() {
		return [
			[At(2, platformLength + 3), At(2, platformLength + 2)]
		];
	}

	function GetDeparturesTiles() {
		return [
			[At(0, platformLength + 2), At(0, platformLength + 1)]
		];
	}
...
class RealSrcRailStation extends RailStation {
	...
	*/
	...
	function GetArrivalsTiles() {
		return [
			[At(1, platformLength + 7), At(1, platformLength + 6), At(1, platformLength + 3)]
		];
	}

	function GetDeparturesTiles() {
		return [
			[At(2, platformLength + 6), At(2, platformLength + 5), At(2, platformLength + 4)]
		];
	}
...
class SimpleRailStation extends RailStation {
	...
	function GetArrivalsTiles() {
		if (platformNum == 1 && !useDepot) {
			return [
				[At(0, platformLength + 0), At(0, platformLength - 1)]
			];
		} else {
			return [
				[At(0, platformLength + 1), At(0, platformLength + 0)]
			];
		}
	}
	...
	function GetDeparturesTiles() {
		if (platformNum == 1) {
			...
			return [
				[At(1, platformLength + 1), At(1, platformLength + 0)]
			];
		}
	}
